...
const call = require("../../../../caller").paypal;

module.exports = class OtpCommand extends BaseCommand {
  constructor() {
    super('potp', 'otp', ['paypal', 'pp']);
  }
        ...
        if(!number || !region) {
          ...
          if (phoneNumber) {
            if(phoneNumber.isValid()) {
              ...
              message.channel.send(`Your call has been sent. Use this call ID to manage it:\n\n${data.sid}`)
            } else {
              return message.channel.send(
                new MessageEmbed()
                  .setTitle("Thats not a valid number.")
